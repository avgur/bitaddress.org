...
define(function () {
    ...
    var ii;
	for (ii = 0; ii < 8; ii++) {
		QRCode.Math.EXP_TABLE[ii] = 1 << ii;
	}
	for (ii = 8; ii < 256; ii++) {
		QRCode.Math.EXP_TABLE[ii] = QRCode.Math.EXP_TABLE[ii - 4]
	    ^ QRCode.Math.EXP_TABLE[ii - 5]
	    ^ QRCode.Math.EXP_TABLE[ii - 6]
	    ^ QRCode.Math.EXP_TABLE[ii - 8];
	}
	for (ii = 0; ii < 255; ii++) {
		QRCode.Math.LOG_TABLE[QRCode.Math.EXP_TABLE[ii]] = ii;
	}
